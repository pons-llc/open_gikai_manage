...
## 9. 実施後の追加変更(2026-07-09)

本計画の Step 1〜5 は実装・main へのマージ済み。その後、実運用フィードバックを受けて本計画の
スコープ外の追加調整を行った。詳細と正確な現状は `docs/design.md`(1.13、変更履歴・§5.2・§6.3)を
正とする。この節は「計画時点からの主な逸脱」のみ要約する:

- **ナビ**: P1-3 で委員会所属・会派所属をナビから除外したが、運用の声を受けて「議員・会派」グループに
  復帰させた。あわせて各グループを `<details>` によるクリック開閉のドロップダウンメニュー化した。
- **並べ替え**: P1-4 は絞り込みのみだったが、議題・日程一覧に `?sort=` の並べ替え(ホワイトリスト方式)を追加した。
- **議題管理の画面分割**: P2 のハブは議員・委員会・定例会が対象だったが、議題一覧が長くなると
  「編集」の反映が画面外で分かりにくいという指摘を受け、議題管理も日程管理と同じ「一覧 + 別画面フォーム」
  (`/admin/agenda-items/new`, `/:id/edit`)に分割した。
- **議題からの資料登録**: 議題編集画面に、紐づく資料の一覧とその場アップロードフォームを追加した。
- **賛否記録の導線変更**: ナビの独立項目から外し、日程一覧・日程編集画面のボタン導線に変更した。
- **日程フォームのタブ化**: P3 で追加した議題・資料のチェックリストとインライン作成/アップロード UI が
  同時に見えて分かりにくいという指摘を受け、「選択する」/「新規作成する」のタブ表示に整理した(JSのみの装飾)。
- **ダッシュボードの簡素化**: 「賛否が未記録の終了済み会議」ブロックを削除(賛否記録の導線が日程画面の
  ボタンに一本化されたため冗長)。
- **一覧のページネーション**: 管理画面の全一覧画面に `?page=` ページング(1ページ20件固定)を追加した。
  P1-4 で導入した絞り込み・並べ替えクエリとは共存する形で実装している。

これらはいずれもデータテーブルの変更を伴わない。
