...
- 版: 1.14
- 作成日: 2026-07-07（1.1: 同日 — コスト管理・会派所属・セキュリティ監査・お知らせテーブルを追加 / 1.2: 同日 — キャッシュ方式をゾーンレベル Cache Everything に変更、Bot Fight Mode を追加、R2 配信方式の判断を明記 / 1.3: 同日 — 議題に予約公開機能を追加 / 1.4: 同日 — 日程と議題を直接紐づける `meeting_agenda_items` を追加 / 1.5: 同日 — 認証を better-auth から D1 + Web Crypto の自前実装に変更 / 1.6: 同日 — 議員ごとの賛否記録 `agenda_item_votes` を追加 / 1.7: 同日 — フェーズ6のセキュリティ監査で発覚した資料ダウンロードの公開状態バイパスを修正、§12未決事項3を解決 / 1.8: 同日 — 管理操作ログ(console.log構造化JSON)を全管理ルートに実装、公開議員一覧に「期」絞り込みを追加、デモデータ投入スクリプトを追加 / 1.9: 同日 — 議題詳細の賛否結果は賛否が記録された会議のみ表示するよう変更、日程一覧の定例会絞り込みを削除（月別カレンダーのみ） / 1.10: 同日 — 公開議題一覧にキーワード検索（`?q=`）を追加、LIKE パターンのエスケープ方針を明記 / 1.11: 2026-07-08 — `docs/admin-ux-improvement-plan.md` を実装。管理画面にフラッシュメッセージ・「登録して続けて入力」・グループ化ナビを追加(P1)、議題/日程/資料一覧に絞り込みを追加(P1-4)、議員・委員会・定例会の詳細ハブページを追加し所属画面をハブに吸収(P2)、日程フォームの議題・資料チェックリストに絞り込み・表示順自動採番・その場アップロードを追加(P3)、議題クイック作成 API `POST /api/admin/agenda-items` を追加(P3-4)、ダッシュボードを「今日やること」型に刷新(P4)。データテーブルの変更なし / 1.12: 2026-07-09 — 実装後の追加フィードバックを反映。管理ナビをクリック開閉のドロップダウンメニュー化し、会派所属・委員会所属を「議員・会派」グループへ復帰(1.11でナビから外していたのを再度追加)/ 議題・日程一覧に並べ替え(`?sort=`、ホワイトリスト方式)を追加 / 議題編集画面から資料を直接アップロードできるように(`POST /api/admin/documents` に `return_to=agenda_item` を追加、検証済み数値IDのみで組み立てるため open redirect にはならない)/ 賛否記録をナビから外し、日程一覧・日程編集画面からのボタン導線に変更 / 議題管理を日程管理と同じ一覧+別画面(`/new`, `/:id/edit`)方式に分割 / 日程フォームの議題・資料セクションを「選択する」/「新規作成する」のタブ表示に整理(JSのみの装飾)。データテーブルの変更なし / 1.13: 2026-07-09 — ダッシュボードから「賛否が未記録の終了済み会議」ブロックを削除(賛否記録への導線は日程画面のボタンに一本化済みのため冗長)。管理画面の全一覧画面(議員・会派・委員会・定例会・議案種別・お知らせ・議題・日程・資料・委員会所属/会派所属横断一覧・賛否記録対象会議一覧)に `?page=` によるページネーションを追加(`src/lib/pagination.ts`、1ページ20件固定、既存の絞り込み・並べ替えクエリと共存)。データテーブルの変更なし / 1.14: 2026-07-09 — 年間200〜400件規模の増加ペースを踏まえ、既存の絞り込みクエリでインデックスが効いていなかった箇所を修正(§3.2.3)。`agenda_items.category`・`committee_memberships`/`faction_memberships` の `term_start` にインデックスを追加(`migrations/0004_perf_indexes.sql`)、`meetings` の年月絞り込みを `substr()` 比較から範囲条件に書き換えて既存インデックスを使えるように修正。ページング方式(OFFSET/LIMIT)自体は現在の規模では妥当と判断し変更していない）
- 元資料: [idea.md](../idea.md)
...
### 3.2.3 管理一覧の絞り込み用インデックス追加（`migrations/0004_perf_indexes.sql`、1.14 追加）

年間200〜400件規模の増加ペースでは OFFSET/LIMIT ページング自体がボトルネックになる可能性は低いが（§9 参照）、既存の絞り込みクエリで一部インデックスが効いていなかったため追加した。
...
> 補足
> - `agenda_items` は `UNIQUE(fiscal_year, category, number)` を既に持つが `fiscal_year` が先頭のため、`category` 単体の絞り込み（`/admin/agenda-items?category=`）には効かない。単体インデックスを追加した。
> - `committee_memberships`/`faction_memberships` は一覧の `ORDER BY term_start DESC, id DESC` に完全一致する複合インデックスとし、ソートも含めてインデックスで完結させる。
> - `meetings` の年月絞り込み（`/admin/meetings?month=`）は元々 `substr(date,1,7) = ?` を使っており、既存の `idx_meetings_date` が使えなかった（関数でラップした列は non-sargable）。これはインデックス追加ではなく、`m.date >= ? AND m.date < ?` の範囲条件に書き換えて既存インデックスを使えるようにした（`src/routes/admin/meetings.tsx`、マイグレーション不要）。
> - `documents.file_name` の部分一致検索（`LIKE '%...%'`）はインデックスを追加しても計画上使われない（前方一致ではないため）。改善するには全文検索（SQLite FTS5）への変更が必要で、現状の規模では見送った。
...
| `GET /admin` | ダッシュボード。「今日やること」型(1.11変更): 直近の日程 / 予約中の議題・お知らせ / ストレージ使用量バー。マスタ件数の一覧表はナビと重複するため廃止した。「賛否が未記録の終了済み会議」ブロックは1.13で削除(賛否記録への導線が日程画面のボタンに一本化され冗長になったため) |
...
| `GET /admin/{memberships,faction-memberships}` | 委員会所属・会派所属の横断一覧(閲覧+編集)。1.11 でナビから外したが、1.12 で「議員・会派」ナビグループ(ドロップダウン)に戻した。ハブページからも「横断一覧を見る」リンクで到達できる |
...
| `GET /admin/agenda-items` `GET /admin/agenda-items/new` `GET /admin/agenda-items/:id/edit` | 議題管理。1.12 で日程管理と同じ「一覧 + 別画面の登録/編集フォーム」方式に分割した(1.11時点では一覧とフォームが同一画面に同居しており、一覧が長いと編集の反映が画面外で分かりにくかったため)。一覧は`?fiscal_year=&category=` で絞り込み(1.11)、`?sort=fiscal_year_desc\|fiscal_year_asc\|number_desc\|number_asc` で並べ替え(1.12、ホワイトリスト方式で ORDER BY を組み立てる)、`新しい議題を登録`ボタンで `/new` へ。編集フォームには紐づく資料の一覧 + その場アップロード(1.12、`POST /api/admin/documents` を通常の multipart フォーム送信で呼び、`return_to=agenda_item` 指定時は編集画面へ戻す)を追加。`published_at` は `<input type="datetime-local">`、未指定なら現在時刻 = 即時公開(§3.4)。登録フォームの「登録して続けて入力」は成功時に年度・種類を引き継いで `/new` へ戻る(1.11→1.12でリダイレクト先を `/new` に変更) |
| `GET /admin/announcements` | お知らせ一覧（予約中バッジ付き）+ 登録/編集フォーム（`published_at` は `<input type="datetime-local">`、未指定なら現在時刻 = 即時公開） |
| `GET /admin/meetings` `GET /admin/meetings/new` `GET /admin/meetings/:id/edit` `POST /admin/meetings/:id/delete` | 日程管理。定例会・議題・資料の紐付け UI。一覧は `?month=&regular_session_id=` で絞り込み(1.11追加)、`?sort=date_desc\|date_asc` で並べ替え(1.12)可能。議題・資料チェックリストはインクリメンタル絞り込み(JS)・表示順自動採番(JS)・資料のその場アップロード・議題のクイック作成に対応(1.11、§6.3・§5.3参照)。1.12 でこれら「選択する」タブと「新規作成する」タブに分離(JSのみの装飾、§6.3参照)。一覧の各行・編集フォームには紐づく議題が1件以上ある会議のみ「賛否を記録」リンク/ボタンを表示する(1.12、旧ナビの `賛否記録` を置き換え) |
| `GET /admin/documents` `POST /admin/documents/:id/delete` | 資料一覧（`?q=&unlinked=on` でファイル名部分一致・議題未紐付けのみ絞り込み、1.11追加）+ アップロードフォーム。削除は他マスタと同じ SSR フォーム POST（§5.3 補足）。1.12 で議題編集画面からも登録できるようになった(上記 `/admin/agenda-items` 参照。ルート自体は変更なし) |
| `GET /admin/votes` | 賛否記録対象の会議一覧（議題が紐づく会議ごとに議題数を表示、1.6追加）。1.12 でナビの独立項目から外し、日程一覧・日程編集画面からのボタン導線に変更した(ルート・画面自体は変更なし) |
...
**一覧のページネーション(1.13追加)**: 上記の管理一覧はすべて `?page=`(1始まり、`src/lib/pagination.ts#parsePage`)による GET ページングに対応し、1ページ20件固定(`ADMIN_PAGE_SIZE`)で `LIMIT`/`OFFSET` を付与する。既存の絞り込み(`?fiscal_year=` 等)・並べ替え(`?sort=`)クエリと共存し、ページ送りリンクは現在のクエリを保持したまま `page` だけを差し替える(`buildPageHref`)。総件数は同じ `WHERE` 条件を共有する別クエリ(`COUNT(*)`)で取得する。JS 不要(`<a>` によるページ送りのみ)。
...
| メソッド/パス | 内容 |
|------|------|
| `POST /api/admin/documents` | `multipart/form-data` でファイル受領 → R2 `put` → `documents` に INSERT。`Accept: application/json` のときはメタデータ JSON を返し、それ以外（JS 無効時の通常フォーム送信）は `/admin/documents` へ 302 リダイレクトする二重対応。1.11 で日程フォームのその場アップロード UI から fetch されるようになった。1.12: 通常フォーム送信時のみ有効な `return_to=agenda_item` を追加(`agenda_item_id` も同時送信されている場合に限る)し、成功時 `/admin/agenda-items/:id/edit` へリダイレクトする。リダイレクト先はクライアントから渡された文字列をそのまま使わず、既に検証済みの数値 ID からサーバ側で組み立てるため open redirect にはならない |
...
| 日程 | `<textarea>` | |
| 議題 | 年度で `<details>` グルーピングしたチェックボックス + 表示順の数値入力 | 1.11: 上部のテキスト入力でインクリメンタル絞り込み(JS、client-side、fetch不要。チェック済み行は絞り込みに関係なく常に表示)。当年度(最新の年度グループ)のみ初期展開。チェックを入れた時点で表示順が0/空なら「現在のチェック済み最大値+1」を自動セット、外したら0に戻す(JS、手入力での上書きも可能)。予約中の議題も選択可能（選択した時点では公開されず、`published_at` 到来まで会議詳細にも出ない）。JS無効時は絞り込み・自動採番なしで全件表示のまま(方針§2準拠)。1.12: このチェックリストと下記「議題(クイック作成)」は「選択する」/「新規作成する」のタブに分離した(下記参照) |
| 会議資料 | 全件チェックボックス + 表示順の数値入力 | 1.11: 議題と同じインクリメンタル絞り込み・表示順自動採番に対応。さらに「ここでアップロード」インライン UI(JS)を追加 — 既存 `POST /api/admin/documents`(Accept: application/json)を fetch で呼び、日程フォーム自体は未送信のままチェックリストに行を動的追加してチェック済み+表示順自動採番にする。JS無効時はインライン UI を隠し、`/admin/documents` へのリンク文言を表示する。1.12: 同様に「選択する」/「アップロードする」タブに分離 |
...
| 賛否記録への導線 | ボタン | 1.12追加。編集中の会議に議題が1件以上紐づく場合のみ「この会議の賛否を記録する」ボタンを表示し `/admin/votes/:id` へ。日程一覧の各行にも同条件で「賛否を記録」リンクを表示(旧ナビ項目「賛否記録」の代替、下記参照) |

**タブ UI(1.12追加)**: 上記「議題」「会議資料」のチェックリストとインライン作成/アップロード UI は、それぞれ「選択する」/「新規作成する(アップロードする)」の2タブに分けて表示する。フォーム送信で保存される範囲(チェックリスト)と、その場で fetch により即座に保存される範囲(クイック作成・アップロード)が同時に見えていると分かりにくいという運用側フィードバックに対応した。タブ切替は JS のみの装飾(`<button data-tab-button>` + `hidden` 属性操作)で、タブナビ自体を JS 実行時にのみ表示する。JS 無効時は旧来どおりチェックリストが常に見え、作成/アップロード UI は隠れたまま(`<noscript>` の代替リンクへ誘導)という 1.11 までの挙動を維持する。作成/アップロードが成功すると自動的に「選択する」タブへ戻り、追加された行をその場で確認できる。

**資料管理 `/admin/documents`**: 一覧（ファイル名/サイズ/議題/登録日、`?q=&unlinked=on` でファイル名部分一致・議題未紐付けのみ絞り込み、1.11追加、GET フォームで JS 不要）、アップロードフォーム（ファイル + 議題セレクト任意）、削除（`POST /admin/documents/:id/delete`）。※当初案にあった「使用中(会議に紐付いている場合)の確認ダイアログに使用先を表示」は実装していない（汎用の削除確認ダイアログのみ）。1.12: 議題編集画面からも同じ API 経由でアップロードできるようになった(下記参照)。

**議題管理 `/admin/agenda-items` `/admin/agenda-items/new` `/admin/agenda-items/:id/edit`**(1.12で一覧とフォームを別画面に分割。1.11時点までは同一画面に同居しており、一覧が長いと「編集」を押しても反映が画面外で分かりにくいという指摘があったため、日程管理と同じ「一覧 + 別画面フォーム」方式に統一した): 種類ラジオの選択に応じて「議案種別セレクト」（種類=議案のとき）と「委員会セレクト」（種類=委員会のとき）を出し分け（JS 無効時は両方表示し、サーバ側バリデーションで担保）。予定公開日時（`published_at`、`<input type="datetime-local">`、未指定なら現在時刻＝即時公開）を持ち、一覧では未来日時の行に「予約中」バッジを付ける（announcements と同一パターン、§3.4）。一覧には `?fiscal_year=&category=` の絞り込み(1.11)と `?sort=` の並べ替え(1.12、年度/番号の昇順降順)を GET フォームで実装(JS 不要)。登録フォームの「登録して続けて入力」ボタンは成功時に年度・種類を引き継いで `/new` へ戻る(1.11で導入、1.12でリダイレクト先を一覧から `/new` に変更)。編集画面には紐づく資料の一覧と、その場アップロードフォームを追加した(1.12。`POST /api/admin/documents` へ通常の `<form>` 送信、`return_to=agenda_item` 指定で送信後に同じ編集画面へ戻る。JS 不要)。

**議員・委員会・定例会の詳細ハブページ**(1.11追加、admin-ux-improvement-plan.md P2): 「1テーブル=1画面」だった所属系マスタを、業務の主役(議員・委員会・定例会)を起点にしたハブへ再編した。「委員会所属」「会派所属」の単独ナビ項目は廃止し(URLは維持)、ハブ内のその場追加フォーム・「終了する」1クリック操作に統合している。1.12: ただし運用の声を受け、ナビからの完全な排除はやめ「議員・会派」グループ(ドロップダウン)の中に両方とも復帰させた(下記ナビ構成参照)。ハブページ自体・URL・ロジック共用は変更なし。

**管理ナビの構成**(1.12刷新): 各グループを `<details>`/`<summary>` によるクリック開閉のドロップダウンメニューにした(JS 不要。JS があると「1つ開いたら他を閉じる」「外側クリックで閉じる」が追加される、`public/assets/admin.js`)。グループは以下の3つ + お知らせ・ログアウト:
- **会議運営**: ダッシュボード / 日程 / 議題 / 資料
- **議員・会派**: 議員 / 会派 / 会派所属 / 委員会所属(1.12でこの2つを復帰)
- **議会マスタ**: 委員会 / 定例会 / 議案種別

賛否記録は 1.12 でナビの独立項目から外し、日程一覧・日程編集画面のボタン導線に変更した(下記参照)。
...
**賛否記録 `/admin/votes`**（1.6 追加）: 議題が紐づく会議の一覧を表示する。`/admin/votes/:meetingId` は縦軸にその会議の議題（`meeting_agenda_items`、表示順）、横軸に議員（議席番号順）を並べたグリッドで、セルごとにセレクトで賛成/反対/棄権/欠席/未記録を選び、ページ全体を一度に送信する（エクセルのような一括入力 UI）。各議題の行には「全員賛成」等のクイック入力ボタンを備える（JS、無効時はセルを1つずつ選択すればよい）。「未記録」を選んだセルは当該会議×議題×議員の賛否記録を削除する（既存の記録を明示的に取り消せるようにするため）。1.11 のダッシュボード改修はスコープ外(手を入れていない、既に完成度が高いため)。1.12: 画面自体・ルートは変更していないが、独立ナビ項目を廃止し、日程一覧の各行(議題が紐づく会議のみ)・日程編集画面(同条件)からのリンク/ボタンでアクセスする方式に変えた(上記「管理ナビの構成」参照)。
